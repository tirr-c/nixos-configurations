  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.nocodb =
      let
        ...
        localPostgresqlUrl = "postgres://localhost/nocodb?host=/run/postgresql";
      ...
      in
      {
        ...
        environment = lib.mkMerge [
          {
            PORT = "${cfg.port}";
            NUXT_PUBLIC_NC_BACKEND_URL = "http://localhost:${cfg.port}";

            NC_JWT_EXPIRES_IN = "${cfg.auth.jwt.expiresIn}";
            NC_ATTACHMENT_FIELD_SIZE = "${cfg.storage.maxAttachmentSize}";
            NC_NON_ATTACHMENT_FIELD_SIZE = "${cfg.storage.maxNonAttachmentSize}";
          }

          (lib.mkIf (cfg.database.backend == "postgresql") {
            NC_DB = localPostgresqlUrl;
          })
